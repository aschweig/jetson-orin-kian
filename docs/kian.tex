...
\centering
\small
\input{benchmark-probe-table}
\caption{TTFT on short filler-word probe prompts (``Ummm'' and ``Okay''),
with responses capped at one token.
Each prompt arrives after 20 conversational turns, so the history is already built up;
the purpose is to verify that no model exhibits pathological latency on minimal user inputs.}
\label{tab:probe}
\end{table}
